...
public class TeamService {
    ...
    public TeamResponse getTeam(Long teamId){
        Team team = teamRepository.findById(teamId).orElseThrow(()-> new EntityNotFoundException("존재 하지 않는 팀입니다."));
    ...
    public TeamResponse createTeam(CreateTeamRequest request, Long employeeId){
        Employee admin = employeeRepository.findById(employeeId).orElseThrow(()-> new EntityNotFoundException("존재 하지 않는 직원입니다."));
        if(admin.getRank().getLevel()<MANAGE_MIN_RANK){
            throw new AccessDeniedException("팀 생성 권한이 없습니다.");
        }
        Employee teamLeader = employeeRepository.findById(request.teamLeaderId()).orElseThrow(()-> new EntityNotFoundException("존재 하지않는 직원입니다."));

        Department department = departmentRepository.findById(request.departmentId()).orElseThrow(()-> new EntityNotFoundException("존재 하지 않는 부서입니다."));
    ...
    public void updateTeamLeader(UpdateTeamLeaderRequest request, Long teamId, Long employeeId){
        ...
        Employee teamLeader = employeeRepository.findById(request.teamLeaderId()).orElseThrow(()-> new EntityNotFoundException("존재 하지 않는 직원입니다."));
